...
class Helper extends CoreHelperHelper
{
    ...
    public function getLabel($product, $label)
    {
        if ($this->isEnabledLabel($label) && $this->isCondition($product, $label)) {
            ...
            $text = (string)$this->getLabelAttr($label, 'text');
    ...
    protected function getBestsellerIds()
    {
        if (empty($this->_bestsellerIds)) {
            ...
            $period = $this->getModuleConfig('label/bestseller_period');
            if ($period) {
                $collection
                    ->addFieldToFilter(
                        'period',
    ...
    public function isConditionSale($product)
    {
        switch ($product->getTypeID()) {
            ...
            case 'configurable':
                ...
                break;
            default:
                $specialPrice = $this->getProductPrice($product,'special');
    ...
    public function templateReplace($product)
    {
        ...
        switch ($productType) {
            case 'grouped':
                ...
                $_special_price = [];
                foreach ($childs as $child) {
                    $specialPrice = $this->getProductPrice($child, 'special');
                    if (!empty($specialPrice)) {
                        $price = $this->getProductPrice($child);
                        $_special_price[] = $specialPrice;
                        $_discount[] = $price - $specialPrice;
                        $_discount_percent[] = round((1 - $specialPrice / $price) * 100);
                    }
                }
            ...
            default:
                ...
                if (!empty($specialPrice)) {
                    ...
                    if ($price > 0) {
                        $discount_percent = round((1 - $specialPrice / $price) * 100) . '%';
                    }
                }
        }
     ...
     * @return float
     */
    protected function getProductPrice($product, $type = '')
    {
        switch ($type) {
            case 'special':
            ...
            case 'final':
                ...
                break;
            case 'calculatedFinal':
                $price = $product->getCalculatedFinalPrice();
                break;
            default:
                $price = $product->getPrice();
        }
        return $this->getTaxHelper()->getTaxPrice($product, $price);
    }
    ...
    {
        return $this->_loadObject(Data::class);
    }
